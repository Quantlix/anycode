"""Run an EvalSuite of scenarios against real LLM providers.
...
Provider selection:
  ...
  - If no provider is available, RuntimeError is raised so the suite fails loud
...
async def run_scenario(scenario: EvalScenario, *, agent: Agent | None = None) -> EvalScenarioResult:
    """Execute one scenario against a real LLM provider."""
