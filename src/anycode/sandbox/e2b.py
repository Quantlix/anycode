...
class E2BSandboxProvider:
    """E2B microVM sandboxes; ``E2B_API_KEY`` stays in SDK configuration."""
    ...
    def _client_or_raise(self) -> Any:
        ...
        try:
            ...
            message = 'E2BSandboxProvider requires the optional "e2b" package. Install it with: pip install "anycode-py[sandbox-e2b]"'
    ...
    async def _resolve(self, handle: SandboxHandle) -> Any:
        ...
        if sandbox is not None:
            return sandbox
        sandbox = await call_maybe_async(self._client_or_raise().connect, handle.id)
        self._sandboxes[handle.id] = sandbox
        return sandbox

    async def create(self, spec: SandboxSpec) -> SandboxCreateResult:
        guard = first_guard_error(
            network_policy_error("e2b", spec, supported=("unrestricted",)),
            secrets_unsupported_error("e2b", spec),
            snapshot_restore_unsupported_error("e2b", spec),
        )
    ...
    async def execute(self, handle: SandboxHandle, command: SandboxCommand) -> SandboxCommandResult:
        try:
            ...
            result = await call_maybe_async(sandbox.commands.run, shell_command(command), timeout=command.timeout_seconds)
    ...
    async def write_file(self, handle: SandboxHandle, path: str, data: bytes) -> SandboxFileResult:
        try:
            sandbox = await self._resolve(handle)
            await call_maybe_async(sandbox.files.write, path, data)
    ...
    async def read_file(self, handle: SandboxHandle, path: str) -> SandboxFileResult:
        try:
            sandbox = await self._resolve(handle)
            data = to_bytes(await call_maybe_async(sandbox.files.read, path))
    ...
    async def cancel(self, handle: SandboxHandle) -> SandboxActionResult:
        try:
            sandbox = await self._resolve(handle)
            await call_maybe_async(sandbox.kill)
    ...
    async def snapshot(self, handle: SandboxHandle, name: str) -> SandboxActionResult:
        ...
        return SandboxActionResult(
            ok=False,
            error=ContractError(code="sandbox_snapshot_unsupported", message="E2B does not expose point-in-time sandbox snapshots."),
        )
    ...
    def capabilities(self) -> SandboxCapabilities:
        return SandboxCapabilities(
            ...
            limitations=(
                ...
                "Cancellation kills the whole sandbox rather than only one command.",
            ),
        )
